...
import { Copy, Download, Edit3, Send, Eye, Code, Smartphone, Monitor } from 'lucide-react'
...
const EmailPreview = ({ selectedEmail, generatedEmails, onSelectEmail }) => {
  ...
  const currentEmail = selectedEmailFromList || selectedEmail || generatedEmails[0]
...
<body>
    <div class="email-header">
        ...
        <p style="margin: 10px 0 0 0; color: #6b7280;">From: ${email.senderInfo.name} <${email.senderInfo.name.toLowerCase().replace(' ', '.')}@${email.senderInfo.company.toLowerCase().replace(' ', '')}.com></p>
        <p style="margin: 5px 0 0 0; color: #6b7280;">To: ${email.recipientInfo.name} <${email.recipientInfo.name.toLowerCase().replace(' ', '.')}@${email.recipientInfo.company.toLowerCase().replace(' ', '')}.com></p>
    </div>
  ...
  const formatEmailAddress = (name, company) => {
    ...
    const emailDomain = company.toLowerCase().replace(' ', '')
    return `${emailName}@${emailDomain}.com`
  }

  const formatFullEmailAddress = (name, company) => {
    return `${name} <${formatEmailAddress(name, company)}>`
  }
                ...
                <div className="p-6">
                  <div className="border-b border-gray-200 pb-4 mb-6">
                    ...
                    <div className="text-sm text-gray-600 space-y-1">
                      <p>
                        <strong>From:</strong> {formatFullEmailAddress(currentEmail.senderInfo.name, currentEmail.senderInfo.company)}
                      </p>
                      <p>
                        <strong>To:</strong> {formatFullEmailAddress(currentEmail.recipientInfo.name, currentEmail.recipientInfo.company)}
                      </p>
                    </div>
                  </div>
